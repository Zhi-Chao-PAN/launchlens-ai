...
describe("<DownloadJsonButton>", () => {
  it("renders a <button type='button'>", () => {
    const html = renderToStaticMarkup(<DownloadJsonButton workspace={FIXTURE} />);
    expect(html).toContain("<button");
    expect(html).toContain('type="button"');
  });

  it("starts in the 'Download .json' state (not yet downloaded)", () => {
    const html = renderToStaticMarkup(<DownloadJsonButton workspace={FIXTURE} />);
  ...
  it("has aria-label='Download workspace as JSON file' in the initial state", () => {
    const html = renderToStaticMarkup(<DownloadJsonButton workspace={FIXTURE} />);
    expect(html).toContain('aria-label="Download workspace as JSON file"');
  });

  it("has a title tooltip explaining the file extension", () => {
    const html = renderToStaticMarkup(<DownloadJsonButton workspace={FIXTURE} />);
    expect(html).toContain('title="Download .json file"');
  });
  ...
  it("applies the supplied className", () => {
    const html = renderToStaticMarkup(<DownloadJsonButton workspace={FIXTURE} className="custom-x" />);
