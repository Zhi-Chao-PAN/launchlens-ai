...
export function EmptyState({
  ...
  return (
    ...
    >
      {icon && (
        <div
          className={`mb-3 flex ${iconSizes[size]} items-center justify-center rounded-full bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500`}
        >
          {icon}
        </div>
      )}
      <h3
        className={`font-medium text-slate-900 dark:text-slate-100 ${titleSizes[size]}`}
      >
        {title}
      </h3>
      {description && (
