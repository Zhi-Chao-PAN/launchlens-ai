...
export function useWorkspaceCommands(options: {
  ...
  return useMemo(() => {
    ...
    actions.push({ id: "filter:active", label: "Filter: show active (unresolved)", description: "Hide supported/refuted hypotheses", category: "Filters", icon: "action", keywords: ["filter", "active", "unresolved", "testing", "untested"], onSelect: () => { onNavigate?.("validation"); window.setTimeout(() => window.dispatchEvent(new CustomEvent("launchlens:filter-status", { detail: "active" })), 80); } });
    actions.push({ id: "filter:decided", label: "Filter: show decided (supported/refuted)", description: "Only show resolved hypotheses", category: "Filters", icon: "action", keywords: ["filter", "decided", "resolved", "done"], onSelect: () => { onNavigate?.("validation"); window.setTimeout(() => window.dispatchEvent(new CustomEvent("launchlens:filter-status", { detail: "decided" })), 80); } });
    actions.push({ id: "filter:all", label: "Filter: show all hypotheses", description: "Clear status filter", category: "Filters", icon: "action", keywords: ["filter", "all", "clear", "reset", "show"], onSelect: () => { onNavigate?.("validation"); window.setTimeout(() => window.dispatchEvent(new CustomEvent("launchlens:filter-status", { detail: "all" })), 80); } });
    actions.push({ id: "filter:clear-all", label: "Clear all filters", description: "Reset status, tag, signal, weight and search", category: "Filters", icon: "action", keywords: ["filter", "clear", "reset", "all"], onSelect: () => { onNavigate?.("validation"); window.setTimeout(() => window.dispatchEvent(new Event("launchlens:clear-filters")), 80); } });
    if (onCopyJson) {
