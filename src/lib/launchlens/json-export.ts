...
export function workspaceFromJson(json: string): WorkspaceImportResult {
  ...
  if (!workspace.generatedAt) {
    ...
    warnings.push("Missing generatedAt —set to current time.");
  }
  if (!workspace.provider) {
    workspace.provider = "mock";
    warnings.push("Missing provider —set to 'mock'.");
  }
  if (!Array.isArray(workspace.mvpScope)) {
    warnings.push("mvpScope is not an array —may be incomplete.");
  }
