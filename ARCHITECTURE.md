...
## 7. R65-R85 Iteration Addendum (2026-06-16)

After the portfolio architecture settled, a sustained iteration pass added the
following cross-cutting pieces that future contributors should know about:

- **Clipboard resilience:** `src/lib/launchlens/clipboard.ts` exports
  `copyTextToClipboard` (async Clipboard API → execCommand fallback, boolean
  result) and `downloadTextFile` (Blob + anchor click). Copy buttons across the
  app (Copy Markdown, Copy JSON, Copy Link, share-link copy, recovery-key copy)
  all route through these so non-secure contexts and denied-permission cases
  degrade to a file download instead of a dead-end toast.
- **`useFocusTrap`** (`src/hooks/use-focus-trap.ts`) provides a small Tab/
  Shift+Tab cycle for modals; wire it with `restoreFocus: false` when the modal
  already has an animated focus-return effect.
- **Overlay handles are idempotent:** `pushOverlay()` returns a dispose whose
  second call is a no-op so React StrictMode double-invoke cannot underflow
  the Escape-stack counter.
- **API error envelope:** every `/api/*` error response has the shape
  `{ code, error }`. Codes live in `src/lib/launchlens/error-codes.ts`; the
  UI looks up friendly text in `src/lib/launchlens/api-errors.ts` so backend
  wording changes do not require hunting through components.
- **Shared-view result shape:** `getSharedWorkspace` returns
  `{ status: "ok" | "revoked" | "not_found" }`; the `/share/[id]` page renders
  a distinct 410-style surface for revoked links instead of a misleading 404.
- **Skip link:** `src/components/skip-link.tsx` is mounted in the root layout
  and jumps to `#main-content`, which every page shell now carries.
- **Tests:** Vitest runs in a pure `node` environment (no jsdom). Unit tests
  cover domain logic only; browser behavior is exercised via Playwright e2e.
  Four quality gates block every push: ESLint (0 warnings), `tsc --noEmit`,
